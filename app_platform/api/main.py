...
app = FastAPI(
    title="Shopnoltd Complete Platform",
    description="Multi-currency payment platform with exchange",
    version="5.0.0",
)
app.add_middleware(
    CORSMiddleware,
...
def get_stats():
    total_usd = sum(
        w["balance"] / EXCHANGE_RATES.get(w["currency"], 1.0) for w in wallets_db.values()
    )
    return {
        "wallets": len(wallets_db),
        "transactions": len(transactions_db),
...
def create_exchange_order(from_currency: str, to_currency: str, amount: float, wallet_id: str):
    ...
    log_action(
        "exchange",
        w["owner_id"],
        {"order_id": order_id, "from": from_currency, "to": to_currency, "amount": amount},
    )
...
def transfer_funds(from_wallet: str, to_wallet: str, amount: float, description: str = "Transfer"):
    ...
    log_action(
        "transfer", from_w["owner_id"], {"from": from_wallet, "to": to_wallet, "amount": amount}
    )
    return {
        "transaction_id": tx_id,
        "amount": amount,
...
def get_transactions(wallet_id: str, limit: int = 50):
    ...
    txs = [
        ...
        if tx.get("wallet_id") == wallet_id
        or tx.get("from_wallet") == wallet_id
        or tx.get("to_wallet") == wallet_id
    ]
...
def submit_kyc(
    user_id: str, full_name: str, document_type: str, document_number: str, country: str = "US"
):
    kyc_id = new_id("kyc")
    users_db[user_id] = {
        "user_id": user_id,
...
PLANS = [
    ...
    },
    {
        ...
        "price": 29.99,
        "currency": "USD",
        "features": ["25 Users", "100GB Storage", "Custom Branding"],
        "trial_days": 14,
        "popular": True,
    },
    {
        ...
        "price": 99.99,
        "currency": "USD",
        "features": ["Unlimited Users", "1TB Storage", "SLA"],
        "trial_days": 30,
    },
]
