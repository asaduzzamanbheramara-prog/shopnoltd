#!/usr/bin/env bash
set -euo pipefail
# Seed all custom platform services with complete source.
# Safe to re-run; only writes files if missing.

ROOT="$(cd "$(dirname "$0")/.." && pwd)"

write() { mkdir -p "$(dirname "$2")"; [ -f "$2" ] || cat > "$2" <<<"$3"; }

# ---------------- payment-service (continuation) ----------------
write bash "$ROOT/platform/payment-service/app/core/config.py" '"""Configuration loaded from env."""
...
write bash "$ROOT/platform/payment-service/app/core/db.py" '"""Database engine and session."""
...
write bash "$ROOT/platform/payment-service/app/core/redis_client.py" '"""Redis client."""
from redis.asyncio import Redis
from app.core.config import settings
redis_client = Redis.from_url(settings.redis_url, decode_responses=True)
'

write bash "$ROOT/platform/payment-service/app/core/security.py" '"""JWT verification via Keycloak JWKS."""
import httpx
...
write bash "$ROOT/platform/payment-service/app/models/models.py" '"""SQLAlchemy models for payments."""
...
write bash "$ROOT/platform/payment-service/app/schemas/schemas.py" '"""Pydantic schemas."""
...
# Providers: each implements create_deposit / handle_webhook / verify
write bash "$ROOT/platform/payment-service/app/providers/base.py" '"""Base payment provider."""
...
write bash "$ROOT/platform/payment-service/app/providers/stripe_provider.py" 'import stripe
...
class StripeProvider(BaseProvider):
    ...
    async def create_deposit(self, tx, return_url=None, **kwargs):
        intent = stripe.PaymentIntent.create(
            amount=int(tx.amount * 100), currency=tx.currency.lower(),
...
write bash "$ROOT/platform/payment-service/app/providers/binance_pay.py" 'import httpx, hmac, hashlib, json, time
from app.providers.base import BaseProvider
from app.core.config import settings
class BinancePayProvider(BaseProvider):
    ...
    def _sign(self, params):
        ...
        return ts, nonce, sig
    async def create_deposit(self, tx, return_url=None, **kwargs):
        params = {"env": {"terminalType": "WEB"}, "orderAmount": float(tx.amount), "currency": tx.currency.upper(),
                  "goods": {"goodsName": f"Shopnoltd deposit {tx.id}", "goodsCategory": "1000"},
                  "orderId": str(tx.id), "returnUrl": return_url or "https://shopnoltd.dpdns.org", "webhookUrl": "https://api.shopnoltd.dpdns.org/api/v1/webhooks/binance"}
        ...
        r.raise_for_status()
        d = r.json()["data"]
        return {"external_id": d["prepayId"], "qr_code": d["qrCodeLink"], "checkout_url": d.get("universalUrl"), "redirect_url": d.get("universalUrl")}
    async def create_withdrawal(self, tx, destination, **kwargs):
        raise NotImplementedError("Binance Pay payouts via Binance API; use manual withdrawal with admin approval")
    async def verify_webhook(self, body, headers):
...
write bash "$ROOT/platform/payment-service/app/providers/bkash.py" 'import httpx, json
from app.providers.base import BaseProvider
from app.core.config import settings
...
class BkashProvider(BaseProvider):
    ...
    async def _token(self):
        ...
        r.raise_for_status()
        self._tok = r.json()["id_token"]; return self._tok
    async def create_deposit(self, tx, return_url=None, **kwargs):
        tok = await self._token()
        amount = f"{float(tx.amount):.2f}"
        async with httpx.AsyncClient() as c:
            r = await c.post(CREATE_URL, json={"mode": "0011", "payerReference": tx.tenant_id, "callbackURL": "https://api.shopnoltd.dpdns.org/api/v1/webhooks/bkash", "amount": amount, "currency": "BDT", "intent": "sale", "merchantInvoiceNumber": str(tx.id)}, headers={"Authorization": tok, "X-APP-Key": settings.bkash_app_key}, timeout=20)
        r.raise_for_status(); d = r.json()
        return {"external_id": d["paymentID"], "redirect_url": d["bkashURL"], "approval_url": d["bkashURL"]}
    async def create_withdrawal(self, tx, destination, **kwargs): raise NotImplementedError("bkash payouts need b2c Payout API; use manual withdrawal")
    async def verify_webhook(self, body, headers): return json.loads(body)
    async def get_status(self, external_id): return "PENDING"
'

write bash "$ROOT/platform/payment-service/app/providers/crypto.py" '"""BTC/ETH/USDT/BNB/SOL/TRX provider using public RPC + admin approval for sweep."""
from app.providers.base import BaseProvider
from app.core.config import settings
import secrets
...
write bash "$ROOT/platform/payment-service/app/providers/manual.py" '"""Manual + bank + Payeer + Nagad + Rocket providers. All use admin approval."""
from app.providers.base import BaseProvider
...
write bash "$ROOT/platform/payment-service/app/providers/registry.py" '"""Provider registry."""
...
# Routers
write bash "$ROOT/platform/payment-service/app/api/wallets.py" 'from fastapi import APIRouter, Depends, HTTPException
...
write bash "$ROOT/platform/payment-service/app/api/deposits.py" 'from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select
from app.core.db import SessionLocal
from app.core.security import verify_token
...
write bash "$ROOT/platform/payment-service/app/api/withdrawals.py" 'from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select
...
write bash "$ROOT/platform/payment-service/app/api/transfers.py" 'from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select
from decimal import Decimal
from app.core.db import SessionLocal
...
write bash "$ROOT/platform/payment-service/app/api/transactions.py" 'from fastapi import APIRouter, Depends, Query
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select
from app.core.db import SessionLocal
from app.core.security import verify_token
...
write bash "$ROOT/platform/payment-service/app/api/exchanges.py" 'from fastapi import APIRouter, Depends, HTTPException
from app.core.security import verify_token
...
write bash "$ROOT/platform/payment-service/app/api/webhooks.py" 'from fastapi import APIRouter, Request, HTTPException
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select
from decimal import Decimal
from datetime import datetime
...
write bash "$ROOT/platform/payment-service/app/api/admin.py" 'from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select
from decimal import Decimal
from datetime import datetime
...
write bash "$ROOT/platform/payment-service/app/api/__init__.py" ''
write bash "$ROOT/platform/payment-service/app/core/__init__.py" ''
write bash "$ROOT/platform/payment-service/app/models/__init__.py" ''
write bash "$ROOT/platform/payment-service/app/schemas/__init__.py" ''
write bash "$ROOT/platform/payment-service/app/providers/__init__.py" ''

write bash "$ROOT/platform/payment-service/app/providers/exchange_client.py" '"""Talks to the local exchange-service for FX rates + conversion."""
import httpx
from datetime import datetime
from app.core.config import settings
...
# Alembic migration
write bash "$ROOT/platform/payment-service/alembic.ini" '[alembic]
script_location = migrations
sqlalchemy.url = postgresql+asyncpg://shopno:shopno@postgres:5432/payments
'

write bash "$ROOT/platform/payment-service/migrations/env.py" 'from logging.config import fileConfig
...
write bash "$ROOT/platform/payment-service/migrations/versions/0001_init.py" '"""initial schema
Revision ID: 0001
"""
