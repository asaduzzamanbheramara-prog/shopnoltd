...
class MoneybagWebhookTests(unittest.TestCase):
    ...
    def test_event_identity_is_persisted_and_duplicate_is_detected(self):
        ...
        event = _record_webhook_event(self.db, event_id="evt_test", event_type="payment.success", transaction_id="txn_test", order_id="order_test", raw_body=body)
        self.assertIsNotNone(event)
        self.db.commit()
        duplicate = _record_webhook_event(self.db, event_id="evt_test", event_type="payment.success", transaction_id="txn_test", order_id="order_test", raw_body=body)
        self.assertIsNone(duplicate)
        self.assertEqual(self.db.query(MoneybagWebhookEvent).filter_by(event_id="evt_test").count(), 1)

    def test_signature_uses_timestamp_and_raw_body(self):
        body = b'{"event_id":"evt_signature"}'
        timestamp = str(int(time.time()))
        signed = timestamp.encode() + b"." + body
        signature = "sha256=" + hmac.new(os.environ["MONEYBAG_WEBHOOK_SECRET"].encode(), signed, hashlib.sha256).hexdigest()
