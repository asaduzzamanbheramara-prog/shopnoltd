...
def db():
    c=sqlite3.connect(DB); c.row_factory=sqlite3.Row; c.execute('create table if not exists peers(id integer primary key,name text not null,public_key text not null unique,address text not null unique,created_at text default current_timestamp)'); return c
...
def ensure_server():
    ...
    try: run('wg','show',IFACE)
        ...
        run('ip','link','add','dev',IFACE,'type','wireguard'); run('wg','set',IFACE,'private-key',str(SERVER_KEY)); run('ip','address','add',SERVER_ADDR,'dev',IFACE,check=False); run('ip','link','set','up','dev',IFACE)
    run('sysctl','-w','net.ipv4.ip_forward=1',check=False)
    r=run('iptables','-t','nat','-C','POSTROUTING','-s',str(NETWORK),'-o','eth0','-j','MASQUERADE',check=False)
    if r.returncode: run('iptables','-t','nat','-A','POSTROUTING','-s',str(NETWORK),'-o','eth0','-j','MASQUERADE',check=False)
    reconcile()
...
def create_peer(p:PeerIn):
    ...
    if ip not in NETWORK or ip==NETWORK.network_address: raise HTTPException(400,'address outside VPN network')
...
def rotate_peer(peer_id:int):
    c=db(); row=c.execute('select * from peers where id=?',(peer_id,)).fetchone()
    if not row: raise HTTPException(404,'peer not found')
    priv=run('wg','genkey').stdout.strip(); pub=get_pub(priv); c.execute('update peers set public_key=? where id=?',(pub,peer_id)); c.commit(); c.close(); reconcile()
