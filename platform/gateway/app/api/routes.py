...
async def traefik_config():
    ...
    for svc, url in SERVICES.items():
        ...
        rid = f"shopno-{svc}"
        http_routers[rid] = {
            "rule": f"Host(`{host}`) && PathPrefix(`/{svc}`)",
            "service": rid,
            "entryPoints": ["websecure"],
            "tls": {"certResolver": "letsencrypt"},
            "middlewares": [f"{rid}-auth"],
        }
        http_services[rid] = {"loadBalancer": {"servers": [{"url": url}]}}
        http_middlewares[f"{rid}-auth"] = {
            "forwardauth": {
                "address": "http://oauth-service.shopno-identity.svc.cluster.local:8080/api/v1/verify"
            }
        }
    return {
        "http": {
