...
async def chat(
    agent_id: str, message: str, user=Depends(current_user), s: AsyncSession = Depends(db)
):
    a = (await s.execute(select(Agent).where(Agent.id == agent_id))).scalar_one_or_none()
    if not a:
        raise HTTPException(404, "agent not found")
    s.add(Conversation(agent_id=a.id, user_id=user["sub"], role="user", content=message))
    await s.commit()
    from app.api.inference import infer
    from app.schemas.schemas import InferIn

    r = await infer(
        InferIn(prompt=f"{a.system_prompt}\n\nUser: {message}\nAssistant:", agent_id=a.id), user
    )
    s.add(Conversation(agent_id=a.id, user_id=user["sub"], role="assistant", content=r.response))
    await s.commit()
