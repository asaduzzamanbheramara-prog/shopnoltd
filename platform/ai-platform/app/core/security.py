...
def _fernet() -> Fernet:
    ...
    if not key:
        raise RuntimeError("AI_KEY_ENCRYPTION_KEY is not configured; refusing to encrypt/decrypt provider credentials")
    try:
...
def decrypt_secret(value: str) -> str:
    ...
    try:
        ...
        raise ValueError("Unable to decrypt provider credential; the configured AI_KEY_ENCRYPTION_KEY may not match the key used to encrypt the stored credential") from exc
...
def user_roles(user: dict) -> set[str]:
    """Return effective Keycloak roles from common JWT claim locations."""
