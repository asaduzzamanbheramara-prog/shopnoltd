...
async def send(
    conv_id: str, body: MsgIn, user=Depends(current_user), s: AsyncSession = Depends(db)
):
    p = (
        await s.execute(
    ...
    m = Message(
        conversation_id=conv_id, sender_id=user["sub"], body=body.body, attachments=body.attachments
    )
    s.add(m)
    await s.commit()
    await s.refresh(m)
