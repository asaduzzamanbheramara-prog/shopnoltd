...
from app.models.models import Certificate, Course, Enrollment
...
async def issue(enrollment_id: str, user=Depends(current_user), s: AsyncSession = Depends(db)):
    ...
    existing = (
        await s.execute(select(Certificate).where(Certificate.enrollment_id == enrollment_id))
    ).scalar_one_or_none()
    if existing:
        return {"id": existing.id, "serial": existing.serial}
    serial = f"SHOPNO-CERT-{secrets.token_hex(6).upper()}"
    c = Certificate(
        enrollment_id=enrollment_id, user_id=user["sub"], course_id=e.course_id, serial=serial
    )
    s.add(c)
    await s.commit()
...
async def pdf(serial: str, s: AsyncSession = Depends(db)):
    cert = (
        await s.execute(select(Certificate).where(Certificate.serial == serial))
    ).scalar_one_or_none()
    if not cert:
        raise HTTPException(404, "not found")
    ...
    p.drawCentredString(4.25 * inch, 9 * inch, "Shopnoltd")
    p.setFont("Helvetica-Bold", 20)
    p.drawCentredString(4.25 * inch, 8 * inch, "Certificate of Completion")
    p.setFont("Helvetica", 14)
    p.drawCentredString(4.25 * inch, 7 * inch, f"Awarded to user {cert.user_id}")
    p.drawCentredString(4.25 * inch, 6.5 * inch, "for completing the course")
    p.setFont("Helvetica-Bold", 16)
    p.drawCentredString(4.25 * inch, 6 * inch, course.title)
    p.setFont("Helvetica", 10)
    p.drawCentredString(4.25 * inch, 3 * inch, f"Serial: {cert.serial}")
