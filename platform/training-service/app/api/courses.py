...
async def create(body: CourseIn, user=Depends(current_user), s: AsyncSession = Depends(db)):
    c = Course(
        tenant_id=user.get("tenant_id", "default"), instructor_id=user["sub"], **body.model_dump()
    )
    s.add(c)
    await s.commit()
...
async def publish(code: str, user=Depends(current_user), s: AsyncSession = Depends(db)):
    c = (
        await s.execute(
            select(Course).where(Course.code == code, Course.instructor_id == user["sub"])
        )
    ).scalar_one_or_none()
    if not c:
        raise HTTPException(404, "not found or not your course")
    c.published = 1
    await s.commit()
