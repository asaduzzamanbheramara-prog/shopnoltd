...
async def global_feed(
    s: AsyncSession = Depends(db), limit: int = Query(50, le=200), offset: int = 0
):
    res = await s.execute(
        select(Post)
...
async def my_feed(
    ...
    f = await s.execute(
        select(Follow.followee_id).where(
            Follow.follower_id == user["sub"], Follow.status == "accepted"
        )
    )
    followees = [r[0] for r in f.all()] + [user["sub"]]
    res = await s.execute(
        select(Post)
        .where(Post.user_id.in_(followees), Post.visibility.in_(["public", "tenant", "friends"]))
        .order_by(desc(Post.published_at))
        .limit(limit)
        .offset(offset)
    )
