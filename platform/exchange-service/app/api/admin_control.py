...
router = APIRouter(prefix="/admin/control-plane", tags=["exchange-database-control-plane"], dependencies=[Depends(require_internal_key)])
...
@router.get("/tables")
async def tables():
    return {"service": "exchange-service", "database_scope": "exchange", "tables": [{"name": n, "writable": False, "financially_sensitive": True} for n in MODELS], "write_policy": "rate and conversion state is service/updater-owned"}

@router.get("/tables/{table_name}")
async def list_rows(table_name: str, limit: int = Query(50, ge=1, le=500), offset: int = Query(0, ge=0), db: AsyncSession = Depends(get_session)):
...
async def analysis(table_name: str, db: AsyncSession = Depends(get_session)):
    ...
    if model is None:
        raise HTTPException(404, f"Unknown exchange table: {table_name}")
    count = await db.execute(select(func.count()).select_from(model))
    return {"table": table_name, "row_count": count.scalar_one(), "writable": False, "financially_sensitive": True, "columns": [{"name": c.name, "type": str(c.type), "nullable": c.nullable, "primary_key": c.primary_key} for c in model.__table__.columns]}

@router.post("/tables/{table_name}")
async def create_row(table_name: str):
    raise HTTPException(403 if table_name in MODELS else 404, "Exchange tables are read-only; use the exchange service or rate updater" if table_name in MODELS else "Unknown exchange table")
