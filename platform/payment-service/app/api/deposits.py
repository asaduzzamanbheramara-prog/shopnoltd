...
async def create_deposit(body: DepositIn, user=Depends(current_user), s: AsyncSession = Depends(db)):
    ...
    if not w:
        w = Wallet(tenant_id=user.get("tenant_id", "default"), user_id=user["sub"], currency=currency, balance=0)
        s.add(w)
        await s.flush()
    tx = Transaction(
        tenant_id=user.get("tenant_id", "default"), user_id=user["sub"], wallet_id=w.id,
        type=TxType.deposit, method=body.method, status=TxStatus.pending,
        amount=body.amount, fee=0, currency=currency, meta=body.metadata,
    )
    s.add(tx)
    await s.flush()
    try:
        ...
        await s.rollback(); raise HTTPException(503, str(exc)) from exc
    except Exception as exc:
        await s.rollback(); raise HTTPException(502, "payment provider request failed") from exc
    tx.external_id = out.get("external_id")
    if out.get("status") in {"failed", "unavailable"}:
        await s.rollback(); raise HTTPException(503, out.get("note") or "Payment provider is unavailable")
    await s.commit()
    return TxOut(id=str(tx.id), type=tx.type, method=tx.method, status=tx.status, amount=float(tx.amount), fee=0,
                 currency=tx.currency, reference=tx.external_id, created_at=tx.created_at.isoformat(), completed_at=None,
                 redirect_url=out.get("redirect_url"), qr_code=out.get("qr_code"), address=out.get("address"), approval_url=out.get("approval_url"))
...
async def get_deposit(tx_id: str, user=Depends(current_user), s: AsyncSession = Depends(db)):
    ...
    res = await s.execute(select(Transaction).where(Transaction.id == transaction_id, Transaction.tenant_id == user.get("tenant_id", "default"), Transaction.user_id == user["sub"]))
    ...
    return TxOut(id=str(tx.id), type=tx.type, method=tx.method, status=tx.status, amount=float(tx.amount), fee=float(tx.fee),
                 currency=tx.currency, reference=tx.external_id, created_at=tx.created_at.isoformat(), completed_at=tx.completed_at.isoformat() if tx.completed_at else None)
