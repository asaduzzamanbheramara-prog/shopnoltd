...
an independently authorized verification result or explicit authenticated
admin review with an audit reason.
"""
...
from app.schemas.schemas import DirectPaymentVerificationIn
...
async def create_account(body: dict, user=Depends(require_admin), s: AsyncSession = Depends(db)):
    ...
    verification_mode = str(body.get("verification_mode", "manual")).lower()
    ...
    if verification_mode not in {"manual", "authorized_feed"}:
        raise HTTPException(400, "verification_mode must be manual or authorized_feed")
    ...
    try:
        ...
        await s.rollback()
        raise HTTPException(409, "receiving account already exists") from exc
    return {"id": str(account.id), "status": account.status, "verification_mode": account.verification_mode}
...
async def admin_submissions(user=Depends(require_admin), s: AsyncSession = Depends(db)):
    rows = (
        await s.execute(
            ...
            .order_by(DirectPaymentSubmission.created_at.desc())
            .limit(200)
        )
    ).scalars().all()
    return {
        "submissions": [
            {
                ...
                "provider": x.provider,
                "txid": x.txid,
                "sender_number": x.sender_number,
                "amount_claimed": float(x.amount_claimed),
                "status": x.status,
                "created_at": x.created_at.isoformat(),
                "verified_at": x.verified_at.isoformat() if x.verified_at else None,
                "verification_data": x.verification_data,
            }
            for x in rows
        ]
    }
...
async def create_intent(body: dict, user=Depends(current_user), s: AsyncSession = Depends(db)):
    ...
    intent = DirectPaymentIntent(
        ...
        amount=amount,
        currency=currency,
        expected_reference=f"SHOPNO-{str(iid).replace('-', '')[:12].upper()}",
...
async def get_intent(intent_id: str, user=Depends(current_user), s: AsyncSession = Depends(db)):
    ...
    intent = await s.scalar(
        select(DirectPaymentIntent).where(
...
async def submit_evidence(intent_id: str, body: dict, user=Depends(current_user), s: AsyncSession = Depends(db)):
    ...
    claimed_amount = Decimal(str(body.get("amount", intent.amount)))
    if claimed_amount <= 0:
    ...
    try:
        ...
        await s.rollback()
        raise HTTPException(409, "this provider TxID has already been submitted") from exc
    return {
...
async def verify_submission(
    submission_id: str,
    body: DirectPaymentVerificationIn,
    user=Depends(require_admin),
    s: AsyncSession = Depends(db),
):
    """Perform an explicit admin-reviewed verification.

    This endpoint is intentionally not presented as automatic provider
    verification. A future authorized provider feed should call the same
    atomic crediting service after cryptographically/verifiably authenticating
    ...
    sub = await s.scalar(select(DirectPaymentSubmission).where(DirectPaymentSubmission.id == sid).with_for_update())
    if not sub:
    ...
    intent = await s.scalar(
        ...
        .where(DirectPaymentIntent.id == sub.intent_id)
        .with_for_update()
    )
    if not intent or intent.status == "expired":
        raise HTTPException(400, "payment intent is not valid")
    if datetime.utcnow() >= intent.expires_at and intent.status != "verified":
        intent.status = "expired"
        await s.commit()
        raise HTTPException(400, "payment intent expired")

    txid = body.provider_transaction_id or body.txid or sub.txid
    if txid != sub.txid:
        raise HTTPException(400, "provider transaction ID / TxID mismatch")
    if Decimal(str(body.amount)) != Decimal(str(intent.amount)):
        sub.status = "failed"
    ...
    account = await s.scalar(select(DirectPaymentAccount).where(DirectPaymentAccount.id == intent.account_id))
    receiver = body.receiver_number.strip()
    if not account or receiver != account.account_number:
        raise HTTPException(400, "receiver account mismatch")

    existing_tx = await s.scalar(
        select(Transaction).where(
            Transaction.external_id == txid,
            Transaction.method == PaymentMethod(intent.provider),
        ).with_for_update()
    )
    if existing_tx:
        if (existing_tx.meta or {}).get("direct_payment_intent_id") != str(intent.id):
            raise HTTPException(409, "provider transaction is already credited to another payment")
        sub.status = "verified"
        sub.transaction_id = existing_tx.id
        sub.verified_at = datetime.utcnow()
        sub.verification_data = {
            "mode": "admin_manual_review",
            "reviewer": user["sub"],
            "reason": body.manual_review_reason,
            "evidence": body.evidence,
        }
        intent.status = "verified"
        await s.commit()
        return {"ok": True, "status": "verified", "transaction_id": str(existing_tx.id)}
    ...
    tx = Transaction(
        ...
        meta={
            ...
            "verification_mode": "admin_manual_review",
            "manual_review_reason": body.manual_review_reason,
            "provider_evidence": body.evidence,
        },
    ...
    try:
        ...
        if existing_tx and (existing_tx.meta or {}).get("direct_payment_intent_id") == str(intent.id):
            return {"ok": True, "status": "verified", "transaction_id": str(existing_tx.id)}
        raise HTTPException(409, "provider transaction was already processed") from exc
