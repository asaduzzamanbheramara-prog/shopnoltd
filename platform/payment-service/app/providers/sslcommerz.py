...
class SSLCommerzProvider(BaseProvider):
    ...
    async def create_deposit(self, tx, return_url=None, **kwargs):
        ...
        payload = {
            ...
            "fail_url": f"{settings.customer_return_url_base}/checkout/complete?ref={reference}&status=failed",
            "cancel_url": f"{settings.customer_return_url_base}/checkout/complete?ref={reference}&status=cancelled",
