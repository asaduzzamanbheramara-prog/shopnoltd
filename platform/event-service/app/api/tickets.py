...
async def register(
    event_id: str, body: TicketIn, user=Depends(current_user), s: AsyncSession = Depends(db)
):
    e = (await s.execute(select(Event).where(Event.id == event_id))).scalar_one_or_none()
    if not e:
        raise HTTPException(404, "event not found")
    qr = qrcode.QRCode(box_size=10, border=2)
    qr.add_data(f"shopno:{event_id}:{user['sub']}")
    qr.make(fit=True)
    img = qr.make_image()
    buf = io.BytesIO()
...
async def my(user=Depends(current_user), s: AsyncSession = Depends(db)):
    ...
    return [
        {"id": t.id, "event_id": t.event_id, "name": t.name, "status": t.status, "qr": t.qr_code}
