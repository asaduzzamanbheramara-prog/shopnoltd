...
from app.models.models import Event, Speaker
...
async def add(
    event_id: str, body: SpeakerIn, user=Depends(current_user), s: AsyncSession = Depends(db)
):
...
async def list_speakers(event_id: str, s: AsyncSession = Depends(db)):
    ...
    return [
        {"id": sp.id, "name": sp.name, "bio": sp.bio, "company": sp.company, "photo": sp.photo}
