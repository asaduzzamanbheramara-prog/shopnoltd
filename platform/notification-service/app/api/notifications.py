...
from app.models.models import NChannel, Notification, NStatus, Template
from app.schemas.schemas import Out, SendIn
...
async def _send(n: Notification, subject: str, body: str):
    try:
        if n.channel == NChannel.email:
            await send_email(n.recipient, subject, body)
        elif n.channel == NChannel.sms:
            await send_sms(n.recipient, body)
        elif n.channel == NChannel.push:
            await send_push(n.recipient, subject, body)
        n.status = NStatus.sent
    except Exception as e:
        n.status = NStatus.failed
        n.error = str(e)
...
async def my_notifications(
    user=Depends(current_user), s: AsyncSession = Depends(db), limit: int = 50
):
    res = await s.execute(
        select(Notification)
