...
from app.models.models import Mailbox, Message
...
async def inbox(
    ...
    res = await s.execute(
        select(Message)
        .where(Message.mailbox_id == mailbox_id, Message.folder == "INBOX")
...
async def send(
    ...
    mb = (
        await s.execute(select(Mailbox).where(Mailbox.id == from_mailbox_id))
    ).scalar_one_or_none()
    if not mb:
        raise HTTPException(404, "from mailbox not found")
    from app.core.mailcow import call_mailcow

    try:
        await call_mailcow(
            "add/message", {"from": mb.full_address, "to": to, "subject": subject, "plain": body}
        )
