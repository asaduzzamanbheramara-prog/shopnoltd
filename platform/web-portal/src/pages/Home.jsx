...
function Card({ service, admin = false }) {
  ...
  const badge = admin ? 'ADMIN' : internal ? 'SHOPNOLTD' : integration ? 'INTEGRATION' : 'EXTERNAL SERVICE'
  const action = admin ? 'Open administration →' : integration ? 'Configure integration →' : internal ? 'Open workspace →' : 'Open service →'
  return <a href={service.url} target={internal ? undefined : '_blank'} rel={internal ? undefined : 'noopener noreferrer'} style={{ display: 'flex', flexDirection: 'column', textDecoration: 'none', color: 'inherit', padding: 20, background: 'white', borderRadius: 16, border: '1px solid #e2e8f0', boxShadow: '0 3px 12px rgba(15,23,42,.06)', minHeight: 175 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}><span style={{ fontSize: 32 }}>{service.icon}</span><span style={{ fontSize: 11, fontWeight: 700, color: integration ? '#7c3aed' : internal ? '#047857' : '#0369a1' }}>{badge}</span></div><h3 style={{ margin: '12px 0 7px' }}>{service.name}</h3><p style={{ margin: 0, color: '#64748b', lineHeight: 1.5, flex: 1 }}>{service.description}</p><span style={{ marginTop: 14, color: '#0284c7', fontWeight: 700 }}>{action}</span></a>
}
...
export default function Home() {
  ...
  return <main style={{ maxWidth: 1180, margin: '0 auto', padding: 'clamp(24px,6vw,52px) clamp(14px,4vw,24px) 80px', boxSizing: 'border-box', fontFamily: 'system-ui,sans-serif' }}>
    <DomainSearch />
    <section style={{ margin: '24px 0 44px', padding: 'clamp(28px,6vw,56px)', borderRadius: 24, background: 'linear-gradient(135deg,#0ea5e9,#0369a1)', color: 'white' }}><div style={{ maxWidth: 800 }}><div style={{ fontSize: 44 }}>🌐</div><h1 style={{ fontSize: 'clamp(38px,7vw,68px)', lineHeight: 1.02, margin: '10px 0 18px' }}>All your tools. One platform.</h1><p style={{ fontSize: 'clamp(17px,2.2vw,21px)', lineHeight: 1.7, opacity: .96 }}>Domains, data collection, communication, calling, social, live media, billing, payments, exchange, AI and developer tools in one Shopnoltd workspace.</p><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 24 }}><a href="/services" style={{ padding: '12px 18px', borderRadius: 10, background: 'white', color: '#0369a1', fontWeight: 800, textDecoration: 'none' }}>Explore services</a><a href="/domain-registration" style={{ padding: '12px 18px', borderRadius: 10, border: '1px solid rgba(255,255,255,.65)', color: 'white', fontWeight: 800, textDecoration: 'none' }}>Register a domain</a></div></div></section>
