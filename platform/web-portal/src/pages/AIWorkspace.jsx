  ...
  return (
    <div style={{ display: 'flex', justifyContent: user ? 'flex-end' : 'flex-start', margin: '18px 0' }}>
      <div style={{ display: 'flex', gap: 12, maxWidth: 'min(760px, 92%)', alignItems: 'flex-start', flexDirection: user ? 'row-reverse' : 'row' }}>
        ...
        <div style={{ whiteSpace: 'pre-wrap', lineHeight: 1.65, fontSize: 15, color: '#1f2937', padding: user ? '10px 14px' : '4px 0' }}>{message.content}</div>
      </div>
    </div>
  )
}
...
export default function AIWorkspace() {
  ...
  async function submit(event) {
    ...
    setChats((current) => current.map((chat) => chat.id === activeId ? { ...chat, title: chat.messages.length ? chat.title : text.slice(0, 48), model: activeModel, messages: [...chat.messages, userMessage] } : chat))
    setPrompt('')
    setLoading(true)
    setError('')
    try {
      const data = await request('/inference', { method: 'POST', body: JSON.stringify({ prompt: text, model: activeModel || null }) })
      const content = data?.response || data?.content || 'The AI service returned an empty response.'
      setChats((current) => current.map((chat) => chat.id === activeId ? { ...chat, messages: [...chat.messages, { id: crypto.randomUUID(), role: 'assistant', content }] } : chat))
    } catch (err) {
  ...
  return (
    <div style={{ height: 'calc(100vh - 56px)', minHeight: 620, display: 'flex', background: '#fff', color: '#111827', overflow: 'hidden' }}>
      ...
      <section className="shopno-ai-main" style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="shopno-ai-composer" style={{ padding: '12px max(18px, calc((100% - 820px) / 2)) 18px', background: '#fff' }}><form onSubmit={submit} style={{ position: 'relative', border: '1px solid #d1d5db', borderRadius: 16, boxShadow: '0 2px 8px rgba(0,0,0,.06)', background: '#fff' }}><textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); submit(e) } }} rows={1} placeholder="Message Shopnoltd AI…" disabled={loading || loadingModels || !models.length} style={{ width: '100%', minHeight: 52, maxHeight: 180, boxSizing: 'border-box', border: 0, outline: 0, resize: 'none', borderRadius: 16, padding: '15px 58px 15px 16px', font: 'inherit', lineHeight: 1.45 }} /><button type="submit" disabled={loading || !prompt.trim() || !models.length} aria-label="Send message" title="Send message" style={{ position: 'absolute', right: 9, bottom: 9, width: 36, height: 36, border: 0, borderRadius: 10, display: 'grid', placeItems: 'center', background: loading || !prompt.trim() || !models.length ? '#d1d5db' : '#111827', color: '#fff', cursor: loading ? 'wait' : 'pointer' }}><Send size={17} /></button></form><div style={{ textAlign: 'center', fontSize: 11, color: '#9ca3af', marginTop: 8 }}>Enter to send · Shift+Enter for a new line · AI responses may be inaccurate</div></div>
      </section>
    </div>
  )
}
