...
const PUBLIC_LINKS = [['Pricing', '/pricing'], ['Create Website', '/create-website'], ['Discover', '/discover'], ['Feed', '/feed'], ['Work', '/work'], ['Blog', '/blog'], ['AI', '/ai'], ['Services', '/services'], ['Domains', '/domain-registration'], ['Android Cloud', 'https://devices.shopnoltd.dpdns.org/']]
...
function Nav() {
  ...
  return <nav style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 12, padding: '12px clamp(14px, 3vw, 24px)', background: '#0ea5e9', color: 'white', boxSizing: 'border-box' }}>
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'flex-end', gap: '8px 16px', minWidth: 0 }}>
      {PUBLIC_LINKS.map(([label, path]) => path.startsWith('http') ? <a key={path} href={path} target="_blank" rel="noopener noreferrer" style={{ color: 'white', textDecoration: 'none', padding: '6px 2px', whiteSpace: 'nowrap', fontWeight: 400 }}>{label}</a> : <Link key={path} to={path} style={{ color: 'white', textDecoration: 'none', padding: '6px 2px', whiteSpace: 'nowrap', fontWeight: location.pathname === path ? 700 : 400 }}>{label}</Link>)}
