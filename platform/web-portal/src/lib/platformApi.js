...
async function storageRequest(path, options = {}) {
  ...
  const headers = { ...(options.headers || {}) }
  if (token) headers.Authorization = `Bearer ${token}`
  const response = await fetch(`${STORAGE_API}${path}`, { ...options, headers })
  const text = await response.text()
  let data = null
  try { data = text ? JSON.parse(text) : null } catch { data = text }
  if (!response.ok) {
...
function storageKeyFromBlogUrl(url) {
  try {
    const parsed = new URL(url, STORAGE_API)
...
export const platformApi = {
  ...
  uploadBlogCover: async (file) => {
    ...
    const owner = encodeURIComponent(user?.id || user?.sub || 'user')
    const key = `blog/${owner}/${crypto.randomUUID()}.${extension}`
    const form = new FormData()
    form.append('file', file)
    const result = await storageRequest(`/api/v1/objects/shopno-blog/${key}`, { method: 'PUT', body: form })
    return { ...result, url: `${STORAGE_API}${result.url}` }
  },
  deleteBlogCover: async (url) => {
    const key = storageKeyFromBlogUrl(url)
    if (!key) return { ok: true }
    return storageRequest(`/api/v1/objects/shopno-blog/${key}`, { method: 'DELETE' })
  },
