...
async def log(
    body: LogIn, request: Request, user=Depends(current_user), s: AsyncSession = Depends(db)
):
    res = await s.execute(
        select(AuditLog)
        .where(AuditLog.tenant_id == user.get("tenant_id", "default"))
...
@router.get("/verify")
async def verify(
    limit: int = Query(1000, le=5000), user=Depends(current_user), s: AsyncSession = Depends(db)
):
