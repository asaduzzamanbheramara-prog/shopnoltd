...
async def load_call(call_id: str, user_id: str, s: AsyncSession):
    ...
    participant = (await s.execute(select(Participant).where(
        Participant.room_id == call.room_id, Participant.user_id == user_id
    ))).scalar_one_or_none()
    if user_id != call.initiator_id and not participant:
...
async def accept_call(call_id: str, user=Depends(current_user), s: AsyncSession = Depends(db)):
    call = await load_call(call_id, user["sub"], s)
