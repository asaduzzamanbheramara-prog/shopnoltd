...
from app.models.models import Participant, Room
...
async def create(body: RoomIn, user=Depends(current_user), s: AsyncSession = Depends(db)):
    ...
    return RoomOut(
        ...
        jitsi_url=f"{settings.jitsi_url}/{r.name}",
        jwt=make_jitsi_jwt(r.name, user["sub"], user.get("name", user["sub"]), True),
    )
    ...
    p = (
        await s.execute(
            select(Participant).where(
                Participant.room_id == r.id, Participant.user_id == user["sub"]
            )
        )
    ).scalar_one_or_none()
    if not p:
        s.add(Participant(room_id=r.id, user_id=user["sub"], role="member"))
        await s.commit()
    return RoomOut(
        id=r.id,
        name=r.name,
        ...
        created_at=r.created_at.isoformat(),
        jitsi_url=f"{settings.jitsi_url}/{r.name}",
        jwt=make_jitsi_jwt(r.name, user["sub"], user.get("name", user["sub"]), False),
    )
...
async def list_rooms(user=Depends(current_user), s: AsyncSession = Depends(db)):
    res = await s.execute(
        select(Room)
        .where(Room.tenant_id == user.get("tenant_id", "default"))
