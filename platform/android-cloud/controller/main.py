...
EMULATOR_IMAGE = os.getenv(
    "ANDROID_CLOUD_EMULATOR_IMAGE",
...
def emulator_pod(session: Session) -> client.V1Pod:
    return client.V1Pod(
        ...
        spec=client.V1PodSpec(
            ...
            security_context=client.V1PodSecurityContext(run_as_non_root=False),
        ...
        spec=client.V1PodSpec(
            ...
            containers=[client.V1Container(
                ...
                env=[
                    ...
                    client.V1EnvVar(name="DISCOVERY_CONTENT", value=discovery),
...
def svc(name: str, selector: dict[str, str], port: int, target: int) -> client.V1Service:
    return client.V1Service(
        ...
        spec=client.V1ServiceSpec(selector=selector, ports=[client.V1ServicePort(name="http", port=port, target_port=target)]),
    )
...
def create_kube_session(session: Session) -> None:
    ...
    core.create_namespaced_service(
        NAMESPACE,
        svc(session.emulator_name, {"shopnoltd.dev/session": session.session_id, "app.kubernetes.io/name": "android-emulator"}, 8554, 8554),
    )
    core.create_namespaced_pod(NAMESPACE, gateway_pod(session))
    core.create_namespaced_service(
        NAMESPACE,
        svc(session.gateway_name, {"shopnoltd.dev/session": session.session_id, "app.kubernetes.io/name": "android-cloud-gateway"}, 8080, 8080),
    )


def delete_kube_session(session: Session) -> None:
    for kind, name in (("pod", session.gateway_name), ("service", session.gateway_name), ("service", session.emulator_name), ("pod", session.emulator_name)):
        try:
            if kind == "pod":
...
async def readyz() -> JSONResponse:
    if not JWT_SECRET:
        return JSONResponse({"status": "not_ready", "reason": "auth_not_configured"}, status_code=503)
    try:
        core.get_namespace(NAMESPACE)
    except Exception as exc:
        return JSONResponse({"status": "not_ready", "reason": str(exc)}, status_code=503)
    return JSONResponse({"status": "ready", "capacity": MAX_SESSIONS})
...
async def create_session(body: CreateSessionIn, user_id: str = Depends(user_from_request)) -> dict[str, Any]:
    ...
    try:
        create_kube_session(session)
    except ApiException as exc:
        delete_kube_session(session)
        raise HTTPException(status_code=503, detail=f"android_cloud_provision_failed:{exc.reason}")
...
def session_response(session: Session) -> dict[str, Any]:
    # The gateway is reached through the public reverse proxy using the opaque session id.
    return {
        "session_id": session.session_id,
        "status": "starting",
        "gateway_url": f"{PUBLIC_GATEWAY_BASE}/{session.session_id}",
        "expires_at": int(session.created_at + SESSION_TTL_SECONDS),
        "turn": {"urls": TURN_URLS, "username": TURN_USERNAME, "credential": TURN_CREDENTIAL} if TURN_URLS else None,
    }
