    ...
    return client.V1Pod(
        ...
        spec=client.V1PodSpec(
            ...
            volumes=[
                client.V1Volume(
                    name="android-data",
                    empty_dir=client.V1EmptyDirVolumeSource(medium="Memory", size_limit="6Gi"),
                )
            ],
        ),
    )
...
def make_gateway_ingress(session: Session) -> client.V1Ingress:
    backend_port = client.V1ServiceBackendPort(number=8080)
    backend_service = client.V1IngressServiceBackend(
        name=session.gateway_name,
        port=backend_port,
    )
    backend = client.V1IngressBackend(service=backend_service)
    ...
    rule = client.V1IngressRule(host=PUBLIC_HOST, http=http)
    annotations = {
