...
async def list_works(
    status: str = Query("open"), limit: int = Query(50, ge=1, le=100),
    offset: int = Query(0, ge=0), s: AsyncSession = Depends(db), user=Depends(current_user)
):
...
async def create_work(body: dict, s: AsyncSession = Depends(db), user=Depends(current_user)):
    ...
    work = Work(
        ...
        requirements=str(body.get("requirements", "")) or None,
...
async def accept_work(work_id: str, s: AsyncSession = Depends(db), user=Depends(current_user)):
    ...
    if work.creator_id == user["sub"]:
        raise HTTPException(400, "creator cannot accept own work")
    if work.status != "open":
        raise HTTPException(409, "work is not open")
    existing = await s.execute(select(WorkAssignment).where(WorkAssignment.work_id == work_id, WorkAssignment.worker_id == user["sub"]))
    ...
    if int(count or 0) + 1 >= work.max_workers:
        work.status = "full"
    await s.commit()
...
async def active_works(s: AsyncSession = Depends(db), user=Depends(current_user)):
    result = await s.execute(
        select(Work, WorkAssignment)
        .join(WorkAssignment, WorkAssignment.work_id == Work.id)
...
async def submit_work(work_id: str, body: dict, s: AsyncSession = Depends(db), user=Depends(current_user)):
    ...
    if not proof:
        raise HTTPException(422, "proof is required")
    work = await s.get(Work, work_id)
    if not work or work.tenant_id != user.get("tenant_id", "default"):
        raise HTTPException(404, "work not found")
    ...
    if not assignment:
        raise HTTPException(409, "accept the work before submitting")
...
async def my_submissions(s: AsyncSession = Depends(db), user=Depends(current_user)):
    ...
    return [{"id": x.id, "work_id": x.work_id, "proof": x.proof, "status": x.status, "review_note": x.review_note, "submitted_at": x.submitted_at.isoformat(), "reviewed_at": x.reviewed_at.isoformat() if x.reviewed_at else None} for x in result.scalars().all()]


@router.get("/works/created/me/submissions")
async def creator_submissions(s: AsyncSession = Depends(db), user=Depends(current_user)):
    result = await s.execute(
        select(WorkSubmission, Work)
        .join(Work, Work.id == WorkSubmission.work_id)
        .where(Work.creator_id == user["sub"])
        .order_by(desc(WorkSubmission.submitted_at))
    )
    return [{"id": x.id, "work": work_dict(w), "worker_id": x.worker_id, "proof": x.proof, "status": x.status, "review_note": x.review_note, "submitted_at": x.submitted_at.isoformat()} for x, w in result.all()]


@router.post("/submissions/{submission_id}/review")
async def review_submission(submission_id: str, body: dict, s: AsyncSession = Depends(db), user=Depends(current_user)):
    ...
    if not work or work.creator_id != user["sub"]:
        raise HTTPException(403, "only the creator can review this submission")
    ...
    await s.commit()
    return {"id": submission.id, "status": submission.status, "settlement": "pending"}
