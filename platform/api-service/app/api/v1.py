...
async def moneybag_webhook(request: Request):
    ...
    headers = {key: value for key, value in request.headers.items() if key.lower().startswith("x-webhook-")}
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            response = await client.post(f"{PAYMENT}/api/v1/webhooks/moneybag", content=body, headers=headers)
...
async def conversations(creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("GET", f"{MESSAGING}/api/v1/conversations", creds.credentials)


@router.post("/conversations")
async def create_conversation(body: dict, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("POST", f"{MESSAGING}/api/v1/conversations", creds.credentials, json=body)


@router.get("/conversations/{conversation_id}")
async def conversation(conversation_id: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("GET", f"{MESSAGING}/api/v1/conversations/{conversation_id}", creds.credentials)


@router.get("/conversations/{conversation_id}/messages")
async def messages(conversation_id: str, limit: int = Query(50, ge=1, le=200), offset: int = Query(0, ge=0), creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("GET", f"{MESSAGING}/api/v1/messages/c/{conversation_id}", creds.credentials, params={"limit": limit, "offset": offset})


@router.post("/conversations/{conversation_id}/messages")
async def send_message(conversation_id: str, body: dict, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("POST", f"{MESSAGING}/api/v1/messages/c/{conversation_id}", creds.credentials, json=body)


@router.post("/conversations/{conversation_id}/read")
async def read_conversation(conversation_id: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("POST", f"{MESSAGING}/api/v1/messages/c/{conversation_id}/read", creds.credentials)


@router.patch("/messages/{message_id}")
async def edit_message(message_id: str, body: dict, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("PATCH", f"{MESSAGING}/api/v1/messages/{message_id}", creds.credentials, json=body)


@router.delete("/messages/{message_id}")
async def delete_message(message_id: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("DELETE", f"{MESSAGING}/api/v1/messages/{message_id}", creds.credentials)


@router.post("/messages/{message_id}/reaction")
async def react_message(message_id: str, reaction: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("POST", f"{MESSAGING}/api/v1/messages/{message_id}/reactions", creds.credentials, params={"reaction": reaction})


@router.delete("/messages/{message_id}/reaction")
async def unreact_message(message_id: str, reaction: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("DELETE", f"{MESSAGING}/api/v1/messages/{message_id}/reactions", creds.credentials, params={"reaction": reaction})


@router.post("/calls/rooms/{room_name}")
async def start_call(room_name: str, call_type: str = "video", creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("POST", f"{MEET}/api/v1/calls/rooms/{room_name}", creds.credentials, params={"call_type": call_type})


@router.post("/calls/{call_id}/{action}")
async def call_action(call_id: str, action: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    if action not in {"accept", "reject", "end"}:
        raise HTTPException(404, "unsupported call action")
    return await call("POST", f"{MEET}/api/v1/calls/{call_id}/{action}", creds.credentials)


@router.get("/calls/{call_id}")
async def call_status(call_id: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("GET", f"{MEET}/api/v1/calls/{call_id}", creds.credentials)


@router.get("/live")
async def live_status(creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("GET", f"{LIVE}/api/v1/streams", creds.credentials)


@router.get("/social/reactions/{post_id}")
async def post_reactions(post_id: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("GET", f"{SOCIAL}/api/v1/reactions/{post_id}", creds.credentials)


@router.put("/social/reactions/{post_id}")
async def add_post_reaction(post_id: str, reaction: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("PUT", f"{SOCIAL}/api/v1/reactions/{post_id}", creds.credentials, params={"reaction": reaction})


@router.delete("/social/reactions/{post_id}")
async def remove_post_reaction(post_id: str, reaction: str, creds: HTTPAuthorizationCredentials = Depends(bearer)):
    return await call("DELETE", f"{SOCIAL}/api/v1/reactions/{post_id}", creds.credentials, params={"reaction": reaction})
